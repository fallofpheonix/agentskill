...
This checkout does not yet contain the promised full platform implementation. The runnable code that exists today is the proxy proof-of-concept in `mitmserver/`; the rest of the repository is design and roadmap material.

**Available runtime artifact:**
- `mitmserver/server.js` - configurable HTTP proxy for local Antigravity traffic inspection
...
cd mitmserver
npm ci
OPEN_ANTIGRAVITY_PROXY_TARGET=http://localhost:3000 npm start
```

Optional environment variables:
- `OPEN_ANTIGRAVITY_PROXY_PORT` - listen port, default `8080`
- `OPEN_ANTIGRAVITY_PROXY_VERBOSE` - set `1` to log non-sensitive headers
