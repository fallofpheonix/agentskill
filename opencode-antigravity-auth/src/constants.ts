...
export function getAntigravityOAuthCredentials(): {
  clientId: string;
  clientSecret: string;
} {
  const clientId = readRequiredEnv(ANTIGRAVITY_CLIENT_ID_ENV);
  const clientSecret = readRequiredEnv(ANTIGRAVITY_CLIENT_SECRET_ENV);
  if (!clientId || !clientSecret) {
    throw new Error(
      `Antigravity OAuth credentials are not configured. Set ${ANTIGRAVITY_CLIENT_ID_ENV} and ${ANTIGRAVITY_CLIENT_SECRET_ENV}.`,
    );
  }
